...
and the toolkit finds the engagement on that workstream. `pwrp_ProactiveEngagementConfigId`
overrides that lookup and is normally empty; set it only when one workstream runs several
active engagements, where the toolkit refuses to guess.

The health check resolves the engagement the same way promotion does, so it fails when
scheduled callback is on and there is nothing to dispatch through, rather than when a box
is empty.
