...
foreach ($api in $definition.apis) {
    ...
    # Creating a custom API generates an sdkmessage row, and that row is what connectors
    # read. It carries its own copy of isprivate and an isactive flag, and clearing
    # isprivate on the API does not propagate to either. An API registered as private
    # therefore leaves an inactive message behind for ever, so the Dataverse connector
    # cannot list or describe it and a cloud flow calling it fails with an empty XRM API
    # error. Correcting the message is the only way back short of deleting the API.
    $apiId = if ($isNew) { $script:apiIds[$api.name] } else { $existing[$api.name] }
    if ($apiId -and -not $WhatIf) {
        $state = Invoke-Dataverse -Method GET -Path ("/api/data/v9.2/customapis($apiId)" +
            "?`$select=customapiid&`$expand=SdkMessageId(`$select=sdkmessageid,isactive,isprivate)")
        $message = $state.SdkMessageId
        if ($message -and (-not $message.isactive -or $message.isprivate)) {
            # No solution header. The sdkmessage is generated by the platform and belongs to
            # it, not to us, so adding it as a component of our solution would be wrong.
            Invoke-Dataverse -Method PATCH -Path "/api/data/v9.2/sdkmessages($($message.sdkmessageid))" `
                -Body @{ isactive = $true; isprivate = $false } | Out-Null
            Write-Host "      reactivated the sdk message" -ForegroundColor Yellow
        }
    }
