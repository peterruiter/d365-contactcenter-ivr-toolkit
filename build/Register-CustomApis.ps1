...
param(
    ...
    # Delete these APIs before registering, so each gets a brand new sdkmessage. The
    # generated message is not editable and keeps whatever it was born with, so an API
    # first registered as private stays unusable to connectors until it is replaced.
    [string[]]$Recreate,
    [switch]$WhatIf
)
...
foreach ($api in $definition.apis) {
    if ($Recreate -contains $api.name -and $existing.ContainsKey($api.name)) {
        Write-Host "  [recreate] $($api.name), for a fresh sdk message" -ForegroundColor Yellow
        if (-not $WhatIf) {
    ...
    # read. It carries its own copy of isprivate, and clearing isprivate on the API does
    # not always propagate. The message cannot be repaired in place: the platform owns it
    # and refuses Update on the sdkmessage table entirely. Deleting the API is the only
    # way to be rid of a bad message, which is what -Recreate is for.
    $apiId = if ($isNew) { $script:apiIds[$api.name] } else { $existing[$api.name] }
    if ($apiId -and -not $WhatIf) {
        $message = (Invoke-Dataverse -Method GET -Path ("/api/data/v9.2/customapis($apiId)" +
            "?`$select=customapiid&`$expand=SdkMessageId(`$select=sdkmessageid,isprivate)")).SdkMessageId
        if ($message -and $message.isprivate) {
            Write-Warning "    its sdk message is still private. Re-run with -Recreate $($api.name)"
        }
    }
